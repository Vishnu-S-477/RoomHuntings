...
database::connection();
$sql=database::$connection->prepare("SELECT productid FROM wishlistfullhouse WHERE clientuserid = ?;");
...
$fullhousearray=[];
for($i=0;$i<count($fullhousepid);$i++){
  $pid =$fullhousepid[$i];
  $pid = $pid['productid'];
  $sql=database::$connection->prepare("SELECT * FROM housedatafullhouse WHERE pid = ?;");
...
$sql=database::$connection->prepare("SELECT productid FROM wishlistflatmate WHERE clientuserid = ?;");
...
$flathousearray=[];
for($i=0;$i<count($flatmatepid);$i++){
  $pid =$flatmatepid[$i];
  $pid = $pid['productid'];
  $sql=database::$connection->prepare("SELECT * FROM housedataflathouse WHERE pid = ?;");
  $sql->bind_param("i",$pid);
  $sql->execute();
  $temps = $sql->get_result();
  while($tem = $temps->fetch_assoc()){
...
$sql=database::$connection->prepare("SELECT productid FROM wishlistpg WHERE clientuserid = ?;");
$sql->bind_param("i",$userid);
$sql->execute();
$variable = $sql->get_result();
$pgpid= [];
...
$pgarray=[];
for($i=0;$i<count($pgpid);$i++){
  $pid =$pgpid[$i];
  $pid = $pid['productid'];
  $sql=database::$connection->prepare("SELECT * FROM housedatapg WHERE pid = ?;");
  $sql->bind_param("i",$pid);
  $sql->execute();
  $temps = $sql->get_result();
  while($tem = $temps->fetch_assoc()){
