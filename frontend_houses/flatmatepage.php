...
$city = isset($_POST['city']) ? trim(strtolower($_POST['city'])) : null;
$area = isset($_POST['area']) ? trim(strtolower($_POST['area'])) : null;
$roomtype = trim($_POST['roomtype']) ?? null;
$eligible = trim($_POST['preferredfor']) ?? null;
$parkingstatus = isset($_POST['parkingstatus']) ? trim(strtolower($_POST['parkingstatus'])) : null;
include '../database/database.php';
database::$db_name = 'poovarasi_RoomHunter';
database::connection();
$sql = "SELECT * FROM housedataflathouse WHERE 1=1 ";
$types="";
$paramater = [];
 if(isset($city)){
  $sql.=" AND city=?";
 ...
 if(isset($eligible)){
  $sql.=" AND allowedtenant LIKE ?";
  $types.="s";
  $paramater[]="%{$eligible}%";
 }
 if(isset($parkingstatus)){
  $sql.=" AND parkingtype LIKE ?";
  $types.="s";
...
<head>
    ...
    <link rel="stylesheet" href="../css_houses/flathousepage.css">
</head>
<body>
...
<div class="option-container" style="cursor:pointer"><img src="../images/menu-icon.png" class="menu-icon-style">
   ...
   <div class="main-container">
    ...
    <form class="property-form" action="flatmatepage.php"  method="post" enctype="multipart/form-data">
    <div class="filter-container">
        ...
        <div class="furnish-input-container">
            ...
            <input id="city-text" type="text" class="text-style" name="city" placeholder="Enter City">
        </div>
        ...
        <div class="furnish-input-container">
            ...
            <select id="parking" name="parkingstatus">
                <option value="2wheller">2wheller</option>
                <option value="4wheller">4wheller</option>
            </select>
        </div>
...
//js code for filter interactive
 ...
 let allbutton = document.querySelectorAll('.add-wishlist-container');
...
function  dataretrieve(variable){
  let productid = variable.dataset.productId;
    ...
    const formData = {
        ...
        fetch('../wishlist/wishlistflathouse.php', {
            method: 'POST',
            headers: {
